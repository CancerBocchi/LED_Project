...
void OLED_Init(void)
{
	...
	OLED_WriteAdd(0xAE); //display off
 
	OLED_WriteAdd(0x20);	//Set Memory Addressing Mode	
	OLED_WriteAdd(0x10);	//00,Horizontal Addressing Mode;01,Vertical Addressing Mode;10,Page Addressing Mode (RESET);11,Invalid
	
	OLED_WriteAdd(0xb0);	//Set Page Start Address for Page Addressing Mode,0-7
 
	OLED_WriteAdd(0xc8);	//Set COM Output Scan Direction
 
	OLED_WriteAdd(0x00); //---set low column address
	OLED_WriteAdd(0x10); //---set high column address
 
	OLED_WriteAdd(0x40); //--set start line address
 
	OLED_WriteAdd(0x81); //--set contrast control register
	OLED_WriteAdd(0xff); //亮度调节 0x00~0xff
 
	OLED_WriteAdd(0xa1); //--set segment re-map 0 to 127
	OLED_WriteAdd(0xa6); //--set normal display
	OLED_WriteAdd(0xa8); //--set multiplex ratio(1 to 64)
	OLED_WriteAdd(0x3F); //
 
	OLED_WriteAdd(0xa4); //0xa4,Output follows RAM content;0xa5,Output ignores RAM content
 
	OLED_WriteAdd(0xd3); //-set display offset
	OLED_WriteAdd(0x00); //-not offset
 
	OLED_WriteAdd(0xd5); //--set display clock divide ratio/oscillator frequency
	OLED_WriteAdd(0xf0); //--set divide ratio
 
	OLED_WriteAdd(0xd9); //--set pre-charge period
	OLED_WriteAdd(0x22); //
 
	OLED_WriteAdd(0xda); //--set com pins hardware configuration
	OLED_WriteAdd(0x12);
 
	OLED_WriteAdd(0xdb); //--set vcomh
	OLED_WriteAdd(0x20); //0x20,0.77xVcc
 
	OLED_WriteAdd(0x8d); //--set DC-DC enable
	OLED_WriteAdd(0x14); //
 
	OLED_WriteAdd(0xaf); //--turn on oled panel
